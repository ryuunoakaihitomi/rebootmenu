...
中加入相应文件名的**空文件或文件夹**.加入wt启用白色主题,加入c可以返回键退出,加入nntc取消显示确认对话直接执行命令,加入dncr启动时不检查root权限,加入urm在启动时不检查root权限的基础上手动开启免root模式.在**Android 7.0**或以上的环境,进入**应用信息**界面,可以看到应用设置的标志(比如一个**齿轮**,在**Android P**上是**应用中的其他设置**选项),进入即可图形化设置选项而无需手动创建删除文件.选项一经修改即时生效.**在一些环境中(比如Android x86)可能需要存储权限以写入配置.**
...
- 增加一个用来调用**系统电源**菜单的图块.
- 长按HOME键快捷锁屏
- **Android P**开始对两个主要界面和设置界面适配**放大镜**,按住选项可以放大查看

## 国内下载渠道

- https://www.coolapk.com/apk/com.ryuunoakaihitomi.rebootmenu

## 准备放弃对Android4.4以下系统的兼容性支持

- 从16到28的跨度实在是太大了...

## 关于Wear OS(旧称Android Wear)

- 本应用检测环境是否为Wear OS的方法是检测是否安装**com.google.android.wearable.app**并且为**系统**应用,一般来说不会判断出错.
- 其实,直到**版本H**的Wear OS才提供了直接弹出的电源菜单.
- Wear OS没有root模式.
- Wear OS不能打开网页链接.~~(现在实现了通过调用zxing应用生成二维码的替代方法)~~
- 通知对Wear OS进行了一定的适配,比如加上了快捷操作项目
- 目前可以通过**ADB调试**(adb install <apk路径>)在智能手表上安装本应用,但这不是建议的做法.**因为目前对Wear OS的适配还是试验性的.**

## 关于本应用的Xposed插件

### 作用

- 一般的root权限管理程序在动作时总有或多或少的延迟,这为本应用的操作带来了粘滞感.通过Xposed注册一个系统服务代替root来进行电源操作,可为本应用的执行操作成功的延迟大为降低.
- 目前仅支持部分root选项,~~计划之后支持更多~~.免root的选项本身不需要优化速度.

### 警告

- 本模块仅支持**Android 6.0.1以上**,不要在之前的系统启用模块!有可能造成本应用失效.
- 受SELinux的限制,Android 8.0开始不允许添加外部的系统服务.所以本应用把应用频率较少的**TvInputService**替换成了自己的服务.**Android TV**(电视设备)不能使用,否则会引发功能异常,其他调用的电视输入相关API的应用也有可能会出现异常.如果有其他Xposed模块也通过相同方式注入服务,**只能有一个应用的服务可以正常工作**.
- 除以上帮助文本外,开机启动后还会显示相关警告的通知.**因为目前本功能还是实验性的,在启用之前请确保自己已经掌握救灾措施.**
- 本应用开机自启会执行root shell启用这个功能,xposed只作为备用

# 错误报告反馈

本应用crash时,如果是手动发送模式,会记录错误信息并且弹出界面请求发送issue.这时需要**Github账号**以取得发送issue的权限.本应用支持**自动填充**(Autofill),Android O或以上系统最好准备支持自动填充的密码管理器.

# 引用组件

本应用使用Crashlytics进行崩溃报告(自动模式),使用FirebaseAnalytics进行事件统计,使用AdMob显示广告为开发者赚取利润.
