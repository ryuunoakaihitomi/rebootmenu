...
- 其实,直到**版本H**的Wear OS才提供了直接弹出的电源菜单.并且Wear OS在**版本H**时才不会导致本应用的使用逻辑错乱.
- Wear OS没有root模式.
- Wear OS不能打开网页链接.
- 目前可以通过**ADB调试**(adb install <apk路径>)在智能手表上安装本应用,但这不是建议的做法.**因为目前对Wear OS的适配还是试验性的**,开发者不负责从应用崩溃到设备爆炸的后果的所有责任.
